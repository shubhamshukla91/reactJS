...
const Product =({id,title,price,description,category,image,rating,count,variant})=>{
...
return(
    ...
    {variant==='home' && <p> Rating: {rating}, Rated By:{count}</p>}
    {variant==='home' &&  <button><Link to={'/product-detail/'+`${id}`}>More information</Link></button>}
    {variant==='detail' &&  <button onClick={handleClick}>Add to Cart</button>}
    </div>
    </>
)
}
